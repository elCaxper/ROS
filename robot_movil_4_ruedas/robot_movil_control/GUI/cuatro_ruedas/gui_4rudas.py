...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.btn_arriba.clicked.connect(self.avanzar)
        self.btn_abajo.clicked.connect(self.retroceder)
        self.btn_der.clicked.connect(self.giro_der)
        self.btn_izq.clicked.connect(self.giro_izq)
    ...
    def retranslateUi(self, Form):
        ...
        self.ch_mantener.setText(_translate("Form", "Giro en el sitio", None))
    ...
    def avanzar(self):
        lineal = Vector3(self.sld_vel.value().real/90.0, 0, 0)
        giro = Vector3(0, 0, 0)
        self.mensaje_vel.angular = giro
        self.mensaje_vel.linear = lineal
        self.pub.publish(self.mensaje_vel)
    ...
    def giro_der(self):
        giro = Vector3(0, 0, -self.sld_vel.value().real / 90.0)
        if self.ch_mantener.isChecked():
            lineal = Vector3(0, 0, 0)
            self.mensaje_vel.linear = lineal
        self.mensaje_vel.angular = giro
        self.pub.publish(self.mensaje_vel)
